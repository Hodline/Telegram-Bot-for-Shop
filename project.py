...
async def pay_callback(query: types.CallbackQuery, state: FSMContext):
    ...
    # Отправляем пользователю сообщение с контактами админа
    admin_contact_button = InlineKeyboardButton(text='Связаться с администратором', url='https://t.me/Zipniks')
    admin_contact_keyboard = InlineKeyboardMarkup().add(admin_contact_button)
    await bot.send_message(query.from_user.id, "Ожидайте, администратор свяжется с вами для завершения оплаты.", reply_markup=admin_contact_keyboard)
...
async def add_item_admin(message: types.Message):
    if message.from_user.id == int(ADMIN_ID):
        await message.answer('Теперь введи название товара:')
...
async def process_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer('Теперь введи цену товара:')
